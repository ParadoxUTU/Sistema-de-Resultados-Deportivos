...
{
    public partial class SubFrmDatosAmonestacion : Form
    {
        ...
        public SubFrmDatosAmonestacion(FrmGestionarEncCol g, Equipo eq)
        {
            InitializeComponent();
            gestionar = g;
        ...
        {
            List<Amonestacion> lista = lgam.devolverAmonestacionesPorDeporte(gestionar.encuentro.idDeporte);
            if (lista.Count != 0)
            {
                foreach (Amonestacion a in lista)
                {
                    cbxNombre.Items.Add(a.nombreAmonestacion);
                }
            }
        }
        ...
        private void SubFrmDatosAmonestacion_Load(object sender, EventArgs e)
        {
