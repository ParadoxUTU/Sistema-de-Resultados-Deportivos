...
namespace SistemaResultadosDeportivos.AccesoADatos
{
    public class DatosCambio
    {
        public List<Cambio> getCambiosByEncuentro(int idEncuentro)
        {
            ...
            String stringSql = "SELECT * FROM Cambios WHERE IdEncuentro = '" + idEncuentro + "';";
            try
            {
                ...
                if (rs.RecordCount > 0)
                {
                    for (int i = 0; i < (int)cantFilas; i++)
                    {
                        ...
                        Cambio cambio = new Cambio(idCambio, minuto, idEncuentro, idEquipo, jugadorEntrante, jugadorSaliente);
                        lista.Add(cambio);
                        rs.MoveNext();
                    }
                }
        ...
        public bool agregarCambio(int minuto, int idEncuentro, int idEquipo, int jugadorEntrante, int jugadorSaliente)
        {
            ...
            {
                ADODB.Connection cn = Conexion.Crear();
                String stringSql = "INSERT INTO Enc_eq_cambios(Minuto, IdEncuentro, IdEquipo, JugadorEntrante, JugadorSaliente) VALUES('" + minuto + "', '" + idEncuentro + "', '" + idEquipo + "', '" + jugadorEntrante + "', '" + jugadorSaliente + "')";
